...
**Stripe refait (nouveau compte `acct_1TrljpD0dQXebFvJ`, accès ancien compte perdu)** : produits + 8 prix live recréés via MCP (lookup keys identiques `sourcing_1..4`, `sourcing_pro_1..4`, HT tax exclusive), webhook `naywa-prod-webhook` → `/api/stripe/webhook` (7 events, version 2026-06-24.dahlia) créé au dashboard, reçus « Paiements réussis » activés, portail client configuré (`bpc_1TrzgID0dQ…`). Clés actualisées sur Vercel par Elyas + redeploy fait. Sonde webhook = 400 signature (secret chargé ✓). `stripe_customer_id` de l'ancien compte nettoyés en DB (3 orgs). **STRIPE_SECRET_KEY validée** (requête atteint `acct_1Trljp…` livemode). **Hotfix post-merge** : le checkout 500ait car `payment_method_types: ["card","sepa_debit"]` était hardcodé et SEPA n'est PAS activé sur le nouveau compte → hardcode retiré (checkout + setup-intent), Stripe utilise les moyens de paiement du dashboard (activer SEPA au dashboard = zéro redeploy) + try/catch sur le flux checkout (fini les 500 sans corps). **Post-hotfix vérifié en prod : POST /api/stripe/checkout → 200 + URL checkout.stripe.com. Chaîne Stripe 100 % opérationnelle.** Chaque paiement d'abonnement génère une facture PDF (portail) + email de reçu.
...
**Ce que la feature apporte (résumé) :**
- **Vivier en 2 zones** (`/workspace/vivier`) : « À classer » ambré en tête + **cartes par secteur** (couleur `lib/sector-color.ts` `sectorHue`, comptage, définition) → clic → liste CV. Recherche = liste plate. Section **« Récemment importés »** scrollable (bande 1 ligne, flèches, repli non destructif). Cartes candidat : barre couleur = secteur, dropdown reclassement en **portal** (SectorReviewControl), × empilé au-dessus d'Ouvrir, badges statut retirés des cartes (gardés en revue d'import via `showStatus`).
- **Classement Nora à l'import** (`sector-classify`, précis, borné) + **« Classer le vivier »** (`/api/sectors/classify-vivier`, mode **décisif**, 150/run). **Création de secteur assistée** (`CreateSectorModal` → `/api/sectors/define` : définition + anti-doublon → stockée `sectors.description` réinjectée dans le classement). Seed 12 secteurs (`lib/sector-defaults.ts`). Gestion renommer/supprimer avec cascade (`/api/sectors/[id]`).
- **Panneau « Matcher le vivier »** (`MatchVivierPanel`) : 3 modes **Intelligent / Personnalisé / Complet** + secteurs cibles proposés par Nora (`/api/jobs/[id]/propose-sectors`, lazy + mémo `target_sectors` → 0 LLM ensuite) en **pills cliquables colorées** (uniquement secteurs présents ≥1 CV, repli >12). Gate `lib/sector-gate.ts` (`partitionByGate`, modes `intelligent`/`personnalise`/`complet`, migration 058) branché dans `/api/jobs/[id]/match` (lit `{mode,target_sectors}`, défaut 'complet' si body absent, mémorise `last_match_mode`, **canary 5%** → bannière « Élargir »). Rappel dernier matching (date+mode+secteurs) sur la fiche mission.
- **Hybrides gérés** partout (`sectors.some(s => cibles.includes(s))`). Compteur carte mission = `score ≥ 55` « N pertinents » (aligné fiche). `computeMissionVisual` couleur = secteurs matchés.
- **Section Brief mission** (`MissionBriefSection`) : brief original + brief client/appel d'offre (migration 057 `jobs.client_brief`).

**Reste / idées futures (non bloquant, déprio Elyas)** : temps-réel vivier entre membres (filtré `user_id`, pré-existant) ; sous-secteurs / split d'un secteur trop gros (option) ; finition visuelle sticky onglets/filtres.

<details><summary>Détail des lots (archive)</summary>
...
- ✅ **Lot 3** (`78d2218`) — **DÉCISION Elyas : secteurs PAS à l'onboarding** (friction inutile si l'user ne matche pas le vivier). À la place, panneau `MatchVivierPanel` au clic sur "Matcher le vivier" : 3 modes **Intelligent / Personnalisé / Complet** (vocab validé, `approfondi`→`personnalise` migration 058) + secteurs cibles proposés par Nora en chips éditables (`POST /api/jobs/[id]/propose-sectors`, **lazy** — seulement si `target_sectors` vide, sinon mémo → 0 LLM ; éditer une chip bascule auto en Personnalisé). Phrase "plus large = plus long". `/api/jobs/[id]/match` lit `{mode, target_sectors}`, applique `partitionByGate` (défaut 'complet' si body absent → bouton "Forcer la relance" ok), mémorise `last_match_mode`+`target_sectors`, **canary 5%** des écartés (bannière "Élargir" si un bon ressort). Pré-filtre travaille sur le pool gaté. `personnalise` = même gate qu'`intelligent`, seuls les secteurs diffèrent.
- ✅ **Lot 4** — vivier en **2 zones**. **4a** (`3be2ddb`) : migration 059 `sectors.description`, `lib/sector-defaults.ts` (12 secteurs + définitions, seedés à la 1ʳᵉ visite via GET /api/sectors idempotent), `sector-classify` prend les définitions (range contre les défs du cabinet), routes `POST /api/sectors/define` (Nora définit + alerte doublon), `PATCH/DELETE /api/sectors/[id]` (rename/delete avec cascade `candidates.sectors`+`jobs.target_sectors`), `POST /api/sectors/classify-vivier` (borné 40/run), panneau match "Ajouter secteur" → **sélecteur des existants** (création canonique = vivier only). **4b** (`f96acce`) : vivier réorganisé — `SectorOverview` (zone "À classer" ambrée en tête + cartes secteur nom/comptage/déf → clic → `SectorDetail` liste CV) + recherche = liste plate, `SectorDetail` (Renommer/Supprimer + reclasse via SectorReviewControl, fix `overflow:hidden` CandidateCard), `CreateSectorModal` (nom → "Demander à Nora" → déf éditable + alerte doublon → créer), bouton "Classer le vivier". Hybrides : un candidat apparaît dans chacun de ses secteurs.

**Correctifs post-review Elyas (mergés)** : classement « Classer le vivier » décisif + plafond 150 ; secteurs seed vides masqués (user-created gardés) ; panneau match en pills colorées (secteurs présents only) ; `SectorReviewControl` dropdown en portal + badges retirés des cartes + cap 6→20 ; alignement actions carte (× SVG au-dessus d'Ouvrir) ; bande « Récemment importés » ; couleur mission = secteurs matchés ; compteur « N pertinents » cohérent (score ≥ 55) ; rappel dernier matching sur la fiche.
