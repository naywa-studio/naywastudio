...
- ⏳ **Lot 4** (ex-5, reprise ici) — vivier en **2 zones** (retour Elyas) : **"À classer"** en haut (petit, seule action requise) + **cartes par secteur** (Nora auto + Validé ensemble, badge discret, "Tout valider ce secteur" optionnel — le dropdown par carte du lot 2b était coupé par `overflow:hidden`, on le remplace par ce layout). **Création de secteur assistée Nora** : nom → définition 1 ligne + garde-fou doublon → validation → stockée (**`sectors.description`**, colonne à ajouter migration 059) et réinjectée dans `sector-classify`. **Seed ~12 secteurs larges** à la 1ʳᵉ visite. Gestion secteurs (renommer/fusionner/supprimer avec cascade `candidates.sectors` + `jobs.target_sectors`) + action "Classer le vivier" (Nora sur les non classés).
