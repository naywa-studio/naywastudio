      ...
      {hoveredIdx !== null && rows[hoveredIdx] && (() => {
        ...
        return (
          <div style={{
            ...
            <Row label="Marge cumulée" value={formatEur(r.margeCumulNominaleEur)} color="var(--nw-text-body)" />
            {r.coutRupture > 0 ? (
              <>
                <div style={{ borderTop: "1px solid var(--nw-border-soft)", margin: "3px 0" }} />
                <Row label="Coût RC" value={`− ${formatEur(r.coutRupture)}`} color="var(--nw-primary)" />
                ...
                <RowSmall label="Si licenciement" value={formatEur(r.margeNetteLicenciementEur)} swatch="var(--nw-text-muted)" dashed />
              </>
            ) : (
              <>
                <Row label="Coût rupture" value="—" color="var(--nw-text-muted)" />
