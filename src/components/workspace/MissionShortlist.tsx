...
const copy = {
  fr: {
    ...
    anonTitle: "CV anonymisés",
    anonNora: "Inclure le résumé Nora",
    anonNoraHint: "Un résumé factuel du profil orienté mission, généré par Nora.",
    ...
    downloadZip: "Télécharger les CV anonymisés",
    downloading: "Génération du zip…",
    selectHint: "Cochez des candidats pour télécharger leurs CV anonymisés en un lot.",
    anonError: "La génération a échoué. Réessayez.",
    optionsSaved: "Réglages enregistrés",
  },
  en: {
    title: (mission: string) => `Shortlist · ${mission}`,
    subtitle: (n: number) =>
    ...
    anonTitle: "Anonymized CVs",
    anonNora: "Include the Nora summary",
    anonNoraHint: "A factual, mission-oriented summary of the profile, generated by Nora.",
    ...
    downloadZip: "Download anonymized CVs",
    downloading: "Building the zip…",
    selectHint: "Tick candidates to download their anonymized CVs as one batch.",
...
export function MissionShortlist({ job, rows, isReadOnly, onLocalUpdate, lang }: Props) {
  ...
  // Anonymisation (lot B/C) : options mission + sélection + téléchargement zip.
  ...
  async function saveOptions(next: { keepNoraSummary: boolean; customText: string }) {
    try {
      ...
      setOptsSaved(true)
      setTimeout(() => setOptsSaved(false), 2000)
    } catch { /* best-effort */ }
  }
  ...
  async function downloadZip() {
    ...
    try {
      ...
      a.href = url
      a.download = `cv-anonymises-${job.title}.zip`
  ...
  return (
    <section aria-label={t.title(job.title)}>
      ...
      {/* Anonymisation — options mission (résumé Nora + message) + rappel sélection */}
      {viewMode === "gallery" && !isReadOnly && shortlisted.length > 0 && (
        <div style={{
          marginBottom: 16, padding: "12px 14px", borderRadius: 12,
          background: "var(--nw-primary-50)", border: "1px solid var(--nw-primary-100)",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.05em", textTransform: "uppercase", color: "var(--nw-primary)", fontFamily: "var(--nw-font-mono)" }}>
              {t.anonTitle}
            </span>
            {optsSaved && <span style={{ fontSize: 11, fontWeight: 600, color: "var(--nw-success)" }}>{t.optionsSaved}</span>}
          </div>
          <label style={{ display: "flex", alignItems: "flex-start", gap: 8, cursor: "pointer" }}>
            <input
              type="checkbox"
              checked={anonNora}
              onChange={(e) => { const v = e.target.checked; setAnonNora(v); void saveOptions({ keepNoraSummary: v, customText: anonMsg }) }}
              style={{ marginTop: 1, width: 15, height: 15, accentColor: "var(--nw-primary)" }}
            />
            <span>
              <span style={{ display: "block", fontSize: 12.5, fontWeight: 700, color: "var(--nw-text)" }}>{t.anonNora}</span>
              <span style={{ display: "block", fontSize: 11, color: "var(--nw-text-muted)", marginTop: 1 }}>{t.anonNoraHint}</span>
            </span>
          </label>
          <div style={{ marginTop: 10 }}>
            <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "var(--nw-text-muted)", marginBottom: 4 }}>{t.anonMessage}</label>
            <input
              type="text"
              value={anonMsg}
              onChange={(e) => setAnonMsg(e.target.value.slice(0, 600))}
              onBlur={() => void saveOptions({ keepNoraSummary: anonNora, customText: anonMsg })}
              placeholder={t.anonMessagePlaceholder}
              style={{
                width: "100%", boxSizing: "border-box", padding: "8px 11px",
                fontSize: 12.5, fontFamily: "inherit", color: "var(--nw-text)",
                background: "white", border: "1px solid var(--nw-border)", borderRadius: 8, outline: "none",
              }}
            />
          </div>
          <p style={{ margin: "10px 0 0", fontSize: 11.5, color: "var(--nw-text-muted)" }}>{t.selectHint}</p>
        </div>
      )}
      ...
      {/* Barre d'action téléchargement groupé (apparaît dès qu'un candidat est coché) */}
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              padding: "10px 18px", cursor: downloading ? "wait" : "pointer", opacity: downloading ? 0.7 : 1,
              whiteSpace: "nowrap",
            }}
          >
...
function ShortlistCard({
  row, isReadOnly, saving, onSelectStage, selectable, isSelected, onToggleSelect, lang, t,
}: {
  row: AssessmentRow
  isReadOnly: boolean
  saving: boolean
