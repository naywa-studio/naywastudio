    ...
    <main style={{
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <SubSection title={t.other}>
                  {cv.languages && cv.languages.length > 0 && (
                    <p style={{ margin: "0 0 8px", fontSize: 13, color: "var(--nw-text-body)" }}>
                      <strong style={{ color: "var(--nw-text)" }}>{t.languagesLabel}</strong> {cv.languages.join(" · ")}
                    </p>
                  )}
                  {cv.certifications && cv.certifications.length > 0 && (
                    <p style={{ margin: 0, fontSize: 13, color: "var(--nw-text-body)" }}>
                      <strong style={{ color: "var(--nw-text)" }}>{t.certificationsLabel}</strong> {cv.certifications.join(" · ")}
                    </p>
                  )}
                </SubSection>
              )}
          ...
          <Section
            title={t.notes}
            right={savingNotes === "saving" ? <SmallStatus color="var(--nw-primary)" label={t.saving} />
...
function CvHealthBar({ cv }: { cv: ParsedCv | null }) {
  ...
  const tier = score == null ? null
    : score >= 75 ? { label: t.cvComplete, fg: "var(--nw-success)", bg: "rgba(34,197,94,0.10)", bd: "rgba(34,197,94,0.3)" }
    : score >= 40 ? { label: t.cvPartial,  fg: "var(--nw-warn)", bg: "rgba(245,158,11,0.10)", bd: "rgba(245,158,11,0.3)" }
    :               { label: t.cvPoor,     fg: "var(--nw-danger-strong)", bg: "rgba(220,38,38,0.10)", bd: "rgba(220,38,38,0.3)" }
