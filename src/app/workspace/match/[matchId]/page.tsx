          ...
          <div style={{
            ...
            {withJob.map((t) => (
              <Link key={t.job!.id} href={`/workspace/pricing/${t.job!.id}`} style={{
                ...
                {t.job!.title}
                {t.score != null && (
                  <span style={{ marginLeft: 6, fontSize: 11, color: "var(--nw-text-muted)" }}>· {t.score}</span>
                )}
              </Link>
            ))}
          </div>
      ...
      >
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <h1 style={{
              ...
              <span style={{ fontWeight: 500, color: "var(--nw-text-muted)", fontSize: 15 }}> — pour </span>
            </h1>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
          ...
          <button
            ...
            title={isReadOnly ? "Lecture seule — souscrivez pour gérer la pipeline" : (match.in_pipeline ? "Retirer de la pipeline" : "Suivre ce candidat dans la pipeline")}
            style={{
        ...
        <div style={{ gridColumn: "1", gridRow: "1", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <section style={{ background: "white", border: "1px solid var(--nw-border-soft)", borderRadius: 16, padding: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              />
              <span style={{ fontSize: 12, color: "var(--nw-text-muted)" }}>€ brut / an{salarySaving ? " · enregistrement…" : ""}</span>
            </div>
            {(() => {
              const target = match.job?.target_gross_salary ?? null
              const ask = match.salary_expectation_brut ?? null
              if (target == null) {
                return <p style={{ margin: "10px 0 0", fontSize: 11.5, color: "var(--nw-text-muted)" }}>Renseignez le salaire cible du poste (dans la mission) pour activer la comparaison.</p>
              }
              if (ask == null) {
                return <p style={{ margin: "10px 0 0", fontSize: 12, color: "var(--nw-text-muted)" }}>Salaire cible du poste : <strong>{target.toLocaleString("fr-FR")} €</strong></p>
              }
              ...
              return (
                <div style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 12, color: "var(--nw-text-muted)" }}>Cible du poste : <strong>{target.toLocaleString("fr-FR")} €</strong></span>
            ...
            {(cv?.years_experience != null || (cv?.languages?.length ?? 0) > 0) && (
              <div style={{
                ...
                {cv?.years_experience != null && (
                  <span>📈 <strong style={{ color: "var(--nw-text-body)" }}>{cv.years_experience} an{cv.years_experience > 1 ? "s" : ""}</strong> d&apos;expérience</span>
                )}
