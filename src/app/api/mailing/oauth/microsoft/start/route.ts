/**
 * GET /api/mailing/oauth/microsoft/start
 *
 * Envoie le sourceur vers l'écran de consentement Microsoft. Répond par une
 ...
 * par l'écran d'autorisation Microsoft — donc lui ferait accorder un accès à sa
...
import { microsoftAuthUrl, microsoftOAuthConfigured, signMicrosoftState } from "@/lib/mailing/oauth-microsoft"
...
export async function GET(req: NextRequest) {
  ...
  if (!microsoftOAuthConfigured()) return back(appUrl, "not_configured")
  ...
  return NextResponse.redirect(microsoftAuthUrl(appUrl, signMicrosoftState(user.id), user.email ?? undefined))
}
