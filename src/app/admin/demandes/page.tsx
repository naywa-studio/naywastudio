              ...
              <article key={batch.batch_id} style={{
                ...
                <header style={{ marginBottom: 14 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 12.5, color: "var(--nw-text-muted)" }}>
                    Demandée par <strong style={{ color: "var(--nw-text-body)" }}>{requesterLabel}</strong>
                    {requesterEmail && (
...
function ValueBlock({ value, dim, highlight }: { value: string | null; dim?: boolean; highlight?: boolean }) {
  return (
    <div style={{
      ...
      {value || <em style={{ color: "var(--nw-text-muted)" }}>(vide)</em>}
    </div>
  )
}
...
function ChangeStatusPill({ status }: { status: Status }) {
  const map: Record<Status, { label: string; color: string; bg: string }> = {
    pending:   { label: "En attente", color: "var(--nw-warn)", bg: "rgba(245,158,11,0.10)" },
    approved:  { label: "Approuvé",   color: "var(--nw-success)", bg: "rgba(34,197,94,0.10)" },
    rejected:  { label: "Refusé",     color: "var(--nw-danger-strong)", bg: "rgba(220,38,38,0.10)" },
    cancelled: { label: "Annulé",     color: "var(--nw-text-muted)", bg: "var(--nw-neutral-100)" },
  }
  const s = map[status]
  return (
    <span style={{
...
function BatchStatusPill({ changes }: { changes: ChangeRow[] }) {
  ...
  if (hasPending) return <Pill label="À traiter" color="var(--nw-warn)" bg="rgba(245,158,11,0.10)" />
  if (allApproved) return <Pill label="Tout approuvé" color="var(--nw-success)" bg="rgba(34,197,94,0.10)" />
  if (allRejected) return <Pill label="Tout refusé" color="var(--nw-danger-strong)" bg="rgba(220,38,38,0.10)" />
  return <Pill label="Décidé" color="var(--nw-text-muted)" bg="var(--nw-neutral-100)" />
}
